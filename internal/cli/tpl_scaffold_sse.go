...
// Agent-mode partial: separate history, stream, and live slots for token-level SSE.
...
<div class="cais-chat-messages-wrap relative flex-1 min-h-0 flex flex-col">
  <div id="chat-messages" class="cais-chat-messages flex-1 overflow-y-auto overflow-x-hidden max-w-full overscroll-contain py-3 min-h-0" data-cais-chat="true">
    <div class="flex flex-col gap-3 min-h-full justify-end">
      ...
      <div
        ...
        {{"{{"}}- end {{"}}"}}
      >
        <div sse-swap="stream" hx-target="#chat-live" hx-swap="innerHTML"></div>
        <div sse-swap="message" hx-target="#chat-stream" hx-swap="beforeend"></div>
        <div sse-swap="thinking" hx-target="#chat-thinking" hx-swap="outerHTML"></div>
      </div>
    </div>
  </div>
